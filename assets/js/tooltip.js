 ...
 *    - .tip-toggle
 * 2) 옵션  
 *    - data-tip-pos="bottom|top|left|right" → 위치 지정 (기본 right)  
...
(function () {
  ...
  function place(el){
    ...
    switch (pos) {
      ...
      case 'top':
        left = r.left + r.width / 2 - t.width / 2 + dx;
        top  = r.top - t.height - GAP + dy;
        TIP_ARROW.style.left = Math.round(t.width / 2 - 4 - dx) + 'px';
        TIP_ARROW.style.top  = (t.height - 3) + 'px';
        break;

      case 'left':
        left = r.left - t.width - GAP + dx;
        top  = r.top + r.height / 2 - t.height / 2 + dy;
        TIP_ARROW.style.left = (t.width - 3) + 'px';
  ...
  function onClick(e){
    ...
    closeTip();
    clickBlock = true;
    setTimeout(() => { clickBlock = false; }, 250);
  }
