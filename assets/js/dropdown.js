/* 드롭다운메뉴 / 다중옵션목록
  - 트리거: [data-dd="template-id"]  (템플릿 id로 메뉴 로드)
    · data-dd-pos="bottom-start|bottom-end|right-start|..." → 위치 옵션
  - 템플릿: <template id="..."> 안에 .dd 구조
  - 서브메뉴: <li class="has-submenu" data-submenu="template-id">
  - 이벤트:
    · 클릭: 열기/닫기, 항목 선택 시 dd:select 이벤트 발생
    · 키보드: ESC, 방향키(↑↓←→) 지원
  - 기능:
    · 메뉴/서브메뉴 뷰포트 보정
    · 서브메뉴 hover 시 열기, 마우스 아웃 시 딜레이 닫기
    · 스크롤/리사이즈 시 자동 위치 재계산
*/
